def check(words):                                                       #words리스트를 매개변수로 받는 함수생성
    count = 0							                                #문자열의 개수를 저장하는 변수
    word = []							                                #동일한 문자열을 저장할 리스트
    for i in range(len(words)):  	   	                                #0부터 문자열 길이만큼 range(n)이면 n - 1까지 보니까 n개 비교하기 위해 리스트 길이만큼 비교한다
...
words = ['aba', 'xyz', 'abcA', '20211020', 'python_p', 'ABCBA', '11']   #words리스트 생성
