...
for i in range(7):
    computer1.append(deck.draw())
    computer2.append(deck.draw())
...
# 특수상황
def special(selected,isComputer2):
    global count  

    #한 번 더: K를 내면 카드를 한 장 더 낼 수 있고, J를 내면 한 턴을 건너뜀
    if(eq(selected[1],'K') or eq(selected[1],'J')):
        if isComputer2:
            turn(players[1],isComputer2)
        else:
            turn(players[0],isComputer2)
            
def turn(hand, isComputer2):
    # 전역 변수 접근
    ...
    if isComputer2:
        ...
        # 가능한 카드
        # 공격 상황(2,A,Joker)
        #공격 방어에 성공 or 카드를 먹어 non_attack상황으로 바뀌었을 땐, 일반상황처럼 getAvailable 호출
